  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Revenue Chart */}
        <div className="lg:col-span-2 bg-white p-8 rounded-3xl border border-navy-100 shadow-sm">
        ...
        <div className="bg-white p-8 rounded-3xl border border-navy-100 shadow-sm">
            ...
            {recentOrders.map((order) => (
              <div key={order.id} className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-navy-900 truncate">{order.user?.name || "Customer"}</p>
                  <p className="text-xs text-navy-400 font-medium tracking-tight">Order #{order.id.slice(-6).toUpperCase()}</p>
                </div>
                <div className="text-right">
                  ...
                  <p className={`text-[10px] font-bold px-2 py-0.5 rounded-full inline-block mt-1 ${
                    order.status === 'DELIVERED' ? 'bg-green-50 text-green-600' : 'bg-saffron/10 text-saffron'
                  }`}>
                    {order.status}
                  </p>
                </div>
          ...
          <button className="w-full mt-8 py-3 rounded-xl border border-navy-100 text-sm font-bold text-navy-600 hover:bg-navy-50 transition-all">
            View All Activity
          </button>
        </div>
      </div>
