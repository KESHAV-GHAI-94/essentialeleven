...
export function AdminCouponList() {
   ...
   const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
         ...
         setNewCoupon({ code: "", type: "PERCENTAGE", discount: 0, minPurchaseAmount: 0, maxDiscountAmount: 0, maxUses: 100, expiresAt: "" });
         loadCoupons();
      } catch (e) {
                  ...
                  <div key={coupon.id} className="bg-white p-6 rounded-3xl border border-navy-100 shadow-sm relative overflow-hidden group">
                     ...
                     <div className="space-y-4">
                        ...
                        <div className="flex gap-8">
                           ...
                           <div>
                              <p className="text-[10px] font-black text-navy-400 uppercase">Min / Max</p>
                              <p className="text-[10px] font-bold text-navy-600">Min: ₹{coupon.minPurchaseAmount}</p>
                              <p className="text-[10px] font-bold text-navy-600">Max: {coupon.maxDiscountAmount ? `₹${coupon.maxDiscountAmount}` : 'No Cap'}</p>
                           </div>
                           <div>
                        ...
                        <div className="pt-4 border-t border-navy-50 flex items-center justify-between">
                           ...
                           className="p-2 hover:bg-red-50 text-red-300 hover:text-red-500 rounded-lg transition-all opacity-0 group-hover:opacity-100"
                        >
                              <Trash2 size={16} />
                           </button>
                        </div>
         ...
         <div className="space-y-6">
            <div className="bg-white p-8 rounded-3xl border border-navy-100 shadow-sm sticky top-24">
               ...
               <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="space-y-1">
                     <label className="text-[10px] font-black text-navy-900 uppercase">Discount Type</label>
                     <select
                        className="w-full bg-navy-50 border-none rounded-xl px-4 py-3 text-sm font-black outline-none focus:ring-1 focus:ring-saffron appearance-none"
                        value={newCoupon.type}
                        onChange={(e) => setNewCoupon({ ...newCoupon, type: e.target.value })}
                     >
                        <option value="PERCENTAGE">Percentage (%)</option>
                        <option value="FIXED">Flat Amount (Direct ₹)</option>
                     </select>
                  </div>

                  <div className="space-y-1">
                     <label className="text-[10px] font-black text-navy-900 uppercase">
                     ...
                     <div className="relative">
                        {newCoupon.type === 'PERCENTAGE' ? (
                           ...
                           <span className="absolute left-3 top-1/2 -translate-y-1/2 text-navy-400 font-black text-sm">₹</span>
                        )}
                        <input
                           required
                           type="number"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                       <label className="text-[10px] font-black text-navy-900 uppercase">Min Purchase (₹)</label>
                       <input
                          type="number"
                          className="w-full bg-navy-50 border-none rounded-xl px-4 py-3 text-sm font-black outline-none focus:ring-1 focus:ring-saffron"
                          value={newCoupon.minPurchaseAmount}
                          onChange={(e) => setNewCoupon({ ...newCoupon, minPurchaseAmount: parseFloat(e.target.value) })}
                       />
                    </div>
                    <div className="space-y-1">
                       <label className="text-[10px] font-black text-navy-900 uppercase">Max Discount (₹)</label>
                       <input
                          type="number"
                  ...
                  <div className="space-y-1">
                     <label className="text-[10px] font-black text-navy-900 uppercase">Total Uses (Usage Limit)</label>
                     <div className="relative">
